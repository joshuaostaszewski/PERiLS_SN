...
# Make meteogram plot
def plot_meteogram(met, probe_id, savedir, special_name=''):
    ''' 
    ...
    '''
    try:
        met.drop_duplicates(inplace=True)
        met = met.reindex(np.arange(met.index[0],met.index[-1], dt.timedelta(minutes=1)))
    except IndexError:
    ...
    ymax,ymin = (np.nanmax(tempf)+3,np.nanmin(dewpf)-3)
    ...
    ymax,ymin = np.round(np.nanmax(mslp))+1,np.round(np.nanmin(mslp))-1
    ...
    ymax,ymin = (np.nanmax(thetav)+2,np.nanmin(thetav)-2)
    ...
    ax3 = axes[3].twinx()
    ymax,ymin = (np.nanmax(thetae)+2,np.nanmin(thetae)-2)
    ...
    # Plot the TTU logo
    im = plt.imread("/Users/jessmcd/Documents/PERiLS_LIVE_WEBPAGE/TTU_Logo_small.tif")
    ...
    plt.savefig('{0}{1}_meteogram{2}.png'.format(savedir,probe_id, special_name),dpi=80,bbox_inches = 'tight')
